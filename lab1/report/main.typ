...
#table(
  ...
  inset: 10pt,
  [Резольвента], [Правило], [подстановка], [Комментарий],
  [{connected(b,u)}], [connected(Node, Node)], [{}], [Случайно взято первое правило, оно нам не подошло, так как узлы не одинаковы],
  ...
  [{edge(b, u)}], [], [], [Находим edge(b, u) в графе],
  [{}], [], [], [True, потому что пустая резольвента],
)
...
#table(
  ...
  inset: 10pt,
  [Резольвента], [Правило], [подстановка], [Комментарий],
  [{connected(a,v)}], [connected(Node, Node)], [{}], [Случайно взято первое правило, оно нам не подошло, так как узлы не одинаковы], // yes
  [{connected(a,v)}], [connected(N1, N2) :- edge(N1, Link1), connected(Link1, N2)], [N1 = a, N2 = v, Link1 = b], [Случайно подошло второе правило, осталось проверить то, что существует нужный нам Link1], // yes
  [{edge(a, b), connected(b,v)}], [connected(N12, N22) :- edge(N11, Link2), connected(Link2, N22)], [N12 = b, N22 = v, Link2 = u], [Случайно подошло второе правило, осталось проверить то, что существует нужный нам Link2], // yes
  [{edge(a, b), edge(b, u), connected(u,v)}], [connected(N13, N23) :- edge(N13, Link3), connected(Link3, N23)], [N13 = u, N23 = v, Link3 = v], [Случайно подошло второе правило, осталось проверить то, что существует нужный нам Link3], // yes
  [{edge(a, b), edge(b, u), edge(u, v), connected(v,v)}], [connected(Node, Node)], [Node = v], [Случайно подошло первое правило], // yes
  [{edge(a, b), edge(b, u), edge(u, v)}], [], [], [Все что осталось в резольвеньте --- это связи, наличие которых нужно проверить, а они есть], // yes
  [{}], [], [], [True, потому что пустая резольвента],
)
